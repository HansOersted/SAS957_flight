...
user_input_end_time = 14100 # Select the end time for the training data
...
subset_df = df[(df["Time (s)"] >= 12990) & (df["Time (s)"] <= closest_end_time)].copy()
...
output_filename = f"training_data_12990s_to_{int(closest_end_time)}s.npz"
