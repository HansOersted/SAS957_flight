...
df = pd.read_csv("tracking_error_dynamics_0s_to_6000s.csv")

for user_input_end_time in range(2220, 2400 + 1, 30):
    closest_end_time = df["Time (s)"].iloc[(df["Time (s)"] - user_input_end_time).abs().argmin()]
    print(f"Selected end time: {closest_end_time} s")

    subset_df = df[(df["Time (s)"] >= 720) & (df["Time (s)"] <= closest_end_time)].copy()
    ...
    output_filename = f"training_data_720s_to_{int(closest_end_time)}s.npz"
